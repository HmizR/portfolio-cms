...
- [x] Add global SEO configuration
- [x] Add shared metadata helpers
- [x] Add page metadata
- [x] Add post metadata
- [x] Add project metadata
- [x] Add publication metadata
...
Milestone 11 — 2026-08-12

- `npm run lint`: PASS
- `npm run typecheck`: PASS
- `npm run test`: PASS (22 files, 49 tests; SEO coverage validates defaults, content overrides, canonical URL generation, social metadata, handle validation, and JSON-LD escaping)
- `npm run test:e2e`: PASS (5 Chromium tests; public coverage verifies canonical metadata, WebSite/Person JSON-LD, sitemap output, and protected-route robots exclusions)
- `npm run build`: PASS (31 generated pages; `/admin/seo`, `/sitemap.xml`, and `/robots.txt` compile successfully)
- `npm run db:generate`: PASS (`drizzle/0010_happy_swordsman.sql`; migration inspected)
- `npm run db:migrate`: PASS against the development PostgreSQL service
- Isolated Playwright database creation, all-migration application, reseeding, and reset: PASS
- `npm run db:check`: PASS against the healthy Compose PostgreSQL service
...
6. Begin Milestone 12 only when it is the requested scope.
...
Milestone 11 handoff decisions:

- Keep `APP_URL` as the centrally validated canonical base URL. Do not add a second mutable base URL in the database.
- Keep site title and default description owned by Appearance; `/admin/seo` owns the managed default social image and optional X/Twitter handle.
- Keep metadata assembly in `src/features/seo/metadata.ts`; route files should supply route-specific facts instead of rebuilding canonical, Open Graph, or Twitter objects.
