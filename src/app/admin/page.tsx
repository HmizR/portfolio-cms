...
import { listPublications } from "@/features/publications/queries";

export const metadata: Metadata = { title: "Overview | PortfolioCMS" };

export default async function AdminPage() {
  const [pages, posts, projects, publications, mediaCount] = await Promise.all([listPages(), listPosts(), listProjects(), listPublications(), countMedia()]);
  const draftCount = [...pages, ...posts, ...projects, ...publications].filter((item) => item.status === "draft").length;
  const recent = [
    ...
    ...publications.map((publication) => ({ href: `/admin/publications/${publication.id}`, id: publication.id, kind: "Publication", title: publication.title, updatedAt: publication.updatedAt })),
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <p className="mt-3 max-w-2xl leading-7 text-slate-600">Manage public identity, content, projects, and academic records from this protected workspace.</p>

    <dl className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
      {[{ label: "Pages", value: pages.length }, { label: "Posts", value: posts.length }, { label: "Projects", value: projects.length }, { label: "Publications", value: publications.length }, { label: "Media", value: mediaCount }, { label: "Draft content", value: draftCount }].map((item) => <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm" key={item.label}><dt className="text-sm font-semibold text-slate-500">{item.label}</dt><dd className="mt-2 font-serif text-3xl font-semibold text-slate-950">{item.value}</dd></div>)}
    </dl>
