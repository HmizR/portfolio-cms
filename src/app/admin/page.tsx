import { FilePlus2, FileText, LockKeyhole, Newspaper } from "lucide-react";
...
export default async function AdminPage() {
  ...
  const draftCount = [...pages, ...posts].filter((item) => item.status === "draft").length;
  const recent = [
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <p className="mt-3 max-w-2xl leading-7 text-slate-600">Manage public identity, standalone pages, and chronological Markdown posts from this protected workspace.</p>

    <dl className="mt-8 grid gap-4 sm:grid-cols-3">
      {[{ label: "Pages", value: pages.length }, { label: "Posts", value: posts.length }, { label: "Draft content", value: draftCount }].map((item) => <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm" key={item.label}><dt className="text-sm font-semibold text-slate-500">{item.label}</dt><dd className="mt-2 font-serif text-3xl font-semibold text-slate-950">{item.value}</dd></div>)}
    </dl>
