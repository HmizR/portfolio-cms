...
export default async function PreviewPost({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!post) notFound();
  const html = await renderMarkdown(post.draftMarkdown ?? post.contentMarkdown);
  return <PublicShell navigation={navigation} site={site}><div className="mb-6 rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900">Private preview · {post.status}</div><PostPresentation coverImageUrl={post.coverImageUrl} excerpt={post.excerpt} html={html} publishedAt={post.publishedAt} tags={post.tags} title={post.title} /></PublicShell>;
}
