...
function revalidateProjectRoutes(slugs: string[]) { invalidateTag(PUBLIC_PROJECTS_CACHE_TAG); for (const slug of new Set(slugs)) revalidatePath(`/projects/${slug}`); revalidatePath("/"); revalidatePath("/projects"); revalidatePath("/cv"); revalidatePath("/admin/projects"); }
...
function revalidateTechnologyRoutes() { invalidateTag(PUBLIC_PROJECTS_CACHE_TAG); revalidatePath("/"); revalidatePath("/projects", "layout"); revalidatePath("/cv"); revalidatePath("/admin/projects", "layout"); }
