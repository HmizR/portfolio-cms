...
function invalidate(slugs: string[]) { updateTag(PUBLIC_PUBLICATIONS_CACHE_TAG); for (const slug of new Set(slugs)) revalidatePath(`/publications/${slug}`); revalidatePath("/publications"); revalidatePath("/admin/publications"); }
