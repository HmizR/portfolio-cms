...
export function MediaPicker({ imagesOnly = false, initialMedia, onSelect, triggerLabel = "Choose media" }: MediaPickerProps) {
  ...
  const visible = useMemo(() => items.filter((item) => (!imagesOnly || item.mimeType.startsWith("image/")) && `${item.originalFilename} ${item.altText}`.toLowerCase().includes(search.toLowerCase())), [imagesOnly, items, search]);
